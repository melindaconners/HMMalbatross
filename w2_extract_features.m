...
tic()
       ...
       for n=[10,15,30] % 10, 15, and 30 sec windows
                    ...
                    % Features to extract for the Hidden Markov Model analysis:    
                    out1 = NaN(nrow,1);     % 1. Dominant Frequency in Dynamic Heave Acceleration
                    out2 = NaN(nrow,1);     % 2. Highest Dominant Frequency in Dynamic Heave Acceleration 
                   ...
                   for j = 1:length(winvec)-1
                      ...
                      if length(find(all(A(ixs,:) == 0,2))) > 2*fs 
                         ...
                         % Use a FAST FOURIER TRANSFORM to identify dominant frequencies in the dynamic heave
                         % acceleration signal:

                         data = A(ixs,3); % Dynamic Heave
                         dataDetr = detrend(data,'constant');
                         dataNum = numel(data);
                         START_FREQ = .05; % Ignore frequencies below .05
                         magFft = NaN;
                         freqSpec = NaN;
                         startInd = NaN;
                         
                         magFft = abs(fft(dataDetr, dataNum))/dataNum; % power spectral density
                         freqSpec = ((0:dataNum/2-1)/dataNum*fs)'; % frequency scale
                            if START_FREQ == 0
                                startInd = 1;
                            else
                                startInd = ceil(length(freqSpec)/fs*START_FREQ*2);
                                startInd = find(freqSpec > START_FREQ, 1, 'first');
                            end
                         [peakAmp, peakInd] = max(magFft(startInd:length(freqSpec))); %maximum power spectral density
                         peakFreq = freqSpec(peakInd+startInd-1);

                         % Analyze frequencies identified in FFT

                         % smooth curve 
                            method = 'gaussian';
                            window = 30;
                            fft_s = smoothdata(magFft(1:length(freqSpec)),method,window);

                            % Find peaks in smoothed curve
                            [pks,locs,wdth,prom] = findpeaks(fft_s,freqSpec,'MinPeakDistance',1.5);
