...
%    set(ax(k), {'Color'}, {[0.9882 0.3059 0.0275],[0.9059 0.7216 0],[0 0.6863    0.7333]})
   if fsrt(k,1)>0,
...
ddiv = div(divk) ;
xlims = [nst ned]/ddiv ;
ylims = [-1 2; -1 5; -1.3 1.0; -40 10];

% line_color = [color1; color2; color3];
...
for k=1:length(X),		% now we are ready to plot
   ...
   if fsrt(k,1)>0,
	...
	set(ax(k),'XLim',xlims) 
%     set(h{k}, 'LineColor', line_color(k,:))
