...
% Function provided by Alexei Vyssotski
...
chi2 = sum( abs( 1 - sum( d.^2 .* repmat( sgns', size( d, 1 ), 1 ), 2 ) )); %chi2 is not a sum of devations!
    %Equation above is questionable
stdsum = sum( ( ones(length(x),1) - sqrt(sum( d.^2, 2))).^2 );%change 2020-02-12, these are real residuals
